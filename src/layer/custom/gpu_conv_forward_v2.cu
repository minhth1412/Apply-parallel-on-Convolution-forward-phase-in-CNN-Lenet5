...
#pragma unroll
        for (int in_idx = 0; in_idx < UNROLL_FACTOR; ++in_idx)
        {
            for (int kernel_row = 0; kernel_row < kernel_size; kernel_row++)
            {
                for (int kernel_col = 0; kernel_col < kernel_size; kernel_col++)
                {
                    int input_row = threadIdx.y + kernel_row;
                    int input_col = threadIdx.x + kernel_col + in_idx * TILE_WIDTH;
                    int kernel_area = kernel_size * kernel_size;

                    accumulator += shared_input[input_row][input_col] * dc_filter[(by * (in_channel * kernel_area)) + ((in_offset + in_idx) * kernel_area) + (kernel_row * kernel_size) + kernel_col];
                }
            }
        }
...
{
	// Optimize version 2: Version 1 + shared memory to store the input data and unroll the input matrix
    printf("\nOptimization version 3: Version 1 + shared memory to store the input data and unroll the input matrix\n");
