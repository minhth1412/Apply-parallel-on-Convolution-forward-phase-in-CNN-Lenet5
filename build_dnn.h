...
Network dnnNetwork()
{
	 ...
	 */
	Network dnn;
	Layer* conv1 = new Conv(1, 28, 28, 6, 5, 5);				// 1 input channel, 28x28 input image, 6 output channels, 5x5 kernel size
	Layer* conv2 = new Conv(6, 12, 12, 16, 5, 5);				// 6 input channels, 12x12 input image, 16 output channels, 5x5 kernel size
...
Network dnnNetwork_gpu()
{
	 ...
	 */
	Network dnn;
	Layer* conv1 = new Conv_gpu(1, 28, 28, 6, 5, 5);				// 1 input channel, 28x28 input image, 6 output channels, 5x5 kernel size
	Layer* conv2 = new Conv_gpu(6, 12, 12, 16, 5, 5);				// 6 input channels, 12x12 input image, 16 output channels, 5x5 kernel size
	Layer* pool1 = new MaxPooling(6, 24, 24, 2, 2, 2);					// 6 input channels, 24x24 input image, 2x2 kernel size, stride = 2
	Layer* pool2 = new MaxPooling(16, 8, 8, 2, 2, 2);					// 16 input channels, 8x8 input image, 2x2 kernel size, stride = 2
	// In this place, it is not necessary to create a flatten layer, because the fully connected layer can automatically flatten the input
	Layer* fc1 = new FullyConnected(pool2->output_dim(), 120);			// pool2->output_dim() = 1024, it returns the flattened dimension of the output of the previous layer (pool2)
